...
import Main from '@/maincomps/UserChildren/StartedJobs.vue';
...
describe("MyJobs.vue", () => {
    ...
    it("correctly calls button for editing" , async() => {
        
        const mock = jest.spyOn(wrapper.vm, 'showDetails');

        const btn1 = wrapper.find({ref:"show_details"})
        btn1.trigger('click');
    ...
    it("correctly renders preview of started jobs" , () => {
        const table = wrapper.find({ref: "started_jobs"})
