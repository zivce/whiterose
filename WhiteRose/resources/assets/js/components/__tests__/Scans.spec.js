...
import Main from '@/maincomps/UserChildren/Scans.vue';
...
describe("Scans.vue", () => {
    ...
    it("correctly renders the scans table" , () => {
        const table = wrapper.find({ref: "scans_table"})
        expect(table).toBeTruthy();

    })
    it("correctly calls click for download button", async() => {
        

        const mock = jest.spyOn(wrapper.vm, 'spy');
        

        const btn1 = wrapper.find({ref:"down_btn"})
        btn1.trigger('click');
